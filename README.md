...
__The goal is to make a script that makes it easy to change the colours of the Mint-L theme.__

__We start from the source files at https://github.com/linuxmint/mint-l-theme and let loose a few scripts to change the colour accent.__
...
* Clone the `mint-l-theme` repository:

```bash
git clone https://github.com/linuxmint/mint-l-theme.git
```
...
* Generate the Mint-L themes with:

```bash
python3 generate-themes.py
```

* Get the `Mint-L-Colora-Theme` scripts; clone the repository with:

```bash
git clone https://github.com/anisbsalah/Mint-L-Colora-Theme.git
```

* Copy/paste the scripts inside `Mint-L-Colora-Theme` to the folder `mint-l-theme` you cloned first. 
...
If all went well, you will find the result in the directory you just cloned (mint-l-theme) in the folder `usr/share/themes`. Now run script No.4 .
